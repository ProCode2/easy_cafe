class CartItem < ApplicationRecord
  ...
  def self.create_items(item_name, item_info)
    existing_item = find_by(menu_item_name: item_name)
    if existing_item
      ...
      create!(menu_item_id: item_info[:id], menu_item_name: item_name,menu_item_quantity: item_info[:quantity].to_i, menu_item_price: (item_info[:price].to_d * item_info[:quantity].to_i))
    end
  end
end
